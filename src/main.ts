...
import {
  CHAT_VIEW_TYPE,
  DEFAULT_SETTINGS,
  type AgenticCopilotSettings,
} from "./constants";
import { type AgentAdapter, type SlashCommand } from "./adapters/types";
import {
  detectAgents,
  getAdapterById,
  type DetectedAgent,
...
export default class AgenticCopilotPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
    ...
    // Notify user
    if (this.activeAdapter) {
      new Notice(`Agentic Copilot: ${this.activeAdapter.displayName} detected`);
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        new AgentSwitchModal(this.app, this.detectedAgents, (agent) => {
          ...
          new Notice(`Switched to ${agent.adapter.displayName}`);
        }).open();
      },
    });
  ...
  private openAgentInTerminal(): void {
    ...
    if (Platform.isMacOS) {
      ...
      exec(`start cmd.exe /k "cd /d "${vaultPath}" && ${binary}"`, (err) => {
        if (err) {
          new Notice(`Failed to open terminal: ${err.message}`);
        }
      });
    ...
    new Notice(`Opening ${this.activeAdapter.displayName} in terminal…`);
  }
