...
export class ClaudeCodeAdapter implements AgentAdapter {
  ...
  buildSpawnArgs(opts: {
    ...
    // When pre-encoded images are provided, use --input-format stream-json
    // to send a multimodal message (text + base64 images) via stdin.
    if (opts.imageData?.length) {
  ...
  async discoverSlashCommands(cwd: string): Promise<SlashCommand[] | null> {
 ...
 */
async function readFrontmatterDesc(filePath: string): Promise<string | null> {
  try {
